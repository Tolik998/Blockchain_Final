  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <div className="glass p-6 space-y-4 text-sm">
        ...
        <button type="button" className="w-full py-2 rounded-lg border border-night-600" onClick={() => void delegateSelf()}>
          Delegate to self
        </button>
        <div>
          <label className="text-xs text-slate-400">Proposal id</label>
          <input className="w-full mt-1 bg-night-800 border border-night-700 rounded-lg px-3 py-2" value={proposalId} onChange={(e) => setProposalId(e.target.value)} />
        </div>
        <p>
          State:{' '}
        ...
        <button
          type="button"
          disabled={isPending || pid === undefined}
